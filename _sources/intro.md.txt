...
Figure {numref}`signal-attributes` illustrates the attributes of a `Signal` class instance: the signal, its individual components as well as their instantaneous frequencies, and the number of components present in the signal for each instant.
